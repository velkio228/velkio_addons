...
class ir_rule(models.Model):
    ...
    def _compute_domain(self, model_name, mode="read"):
        ...
        read_value = True
        self._cr.execute("SELECT state FROM ir_module_module WHERE name='velkio_simplify_access_management'")
        ...
        value = self._cr.execute(
            """SELECT value from ir_config_parameter where key='uninstall_velkio_simplify_access_management' """)
        value = self._cr.fetchone()
        if not value:
            value = self._cr.execute("""select state from ir_module_module where name = 'velkio_simplify_access_management'""")
            value = self._cr.fetchone()
