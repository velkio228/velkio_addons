...
class ir_ui_view(models.Model):
    ...
    def _postprocess_tag_field(self, node, name_manager, node_info):
        ...
        # ── Globally disable Create / Quick-Create / Edit / Open on every
        #    Many2one & Many2many field for users whose rule has
        #    "Disable Relational Create/Edit" enabled. ─────────────────────────
        try:
            if node.tag == 'field':
                field_name = node.get('name')
                field = name_manager.model._fields.get(field_name) if field_name else None
                if field is not None and field.type in ('many2one', 'many2many') \
                        and self.env['access.management']._has_relational_create_edit_restriction():
                    options_dict = {}
                    if node.get('options'):
                        try:
                            options_dict = ast.literal_eval(node.get('options'))
                        except Exception:
                            options_dict = {}
                    options_dict.update({
                        'no_create': True,
