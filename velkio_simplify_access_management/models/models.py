...
class BaseModel(models.AbstractModel):
    ...
    def search_read(self, domain=None, fields=None, offset=0, limit=None, order=None, **read_kwargs):
        # Merged from the former "Advanced Web Domain Widget" module: expose the
        # record's company name to the domain-widget record picker.
...
{resolution_info}""".format(operation_error=operation_error, resolution_info=resolution_info)
    ...
    def unlink(self):
        value = self.env['ir.config_parameter'].sudo().search([('key', '=', 'uninstall_velkio_simplify_access_management')],
                                                              limit=1).value
        if not value:
            for rec in self:
                if rec._name:
    ...
    #     value = self.env['ir.config_parameter'].sudo().search([('key','=','uninstall_velkio_simplify_access_management')],limit=1).value
    #     if not value:
    #         if self._name:
