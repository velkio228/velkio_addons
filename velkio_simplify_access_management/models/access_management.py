...
class access_management(models.Model):
    ...
    disable_relational_create_edit = fields.Boolean(
        string='Disable Relational Create/Edit',
        help="When enabled, every Many2one and Many2many field (on every model) "
             "has its Create, Quick-Create and 'Create & Edit' options removed, "
             "and the open-record (internal link) disabled, for the users in "
             "this rule. No per-field configuration is required.",
    )
    ...
    self_module_menu_ids = fields.Many2many('ir.ui.menu', 'access_management_ir_ui_self_module_menu',
                                            ...
                                            default=lambda self: self.env.ref('velkio_simplify_access_management.main_menu_velkio_simplify_access_management'))
    ...
    #     access_menu_id = self.env.ref('velkio_simplify_access_management.main_menu_velkio_simplify_access_management')
    ...
    def _has_relational_create_edit_restriction(self):
        """True when the current user has at least one active rule (for the
        current company) with ``disable_relational_create_edit`` set.

        Cached per user/company (skipped in ``--dev xml`` mode, like the rest
        of this module); the registry cache-clear already done in
        create/write/unlink invalidates it whenever a rule changes.
        """
        return bool(self.sudo().search_count([
            ('user_ids', 'in', self.env.uid),
            ('company_ids', 'in', self.env.company.id),
            ('active', '=', True),
            ('disable_relational_create_edit', '=', True),
        ]))
